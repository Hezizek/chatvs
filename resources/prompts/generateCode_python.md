...
3. **导入语句规则（非常重要）**:
   * **数据结构导入**: 
     - 项目的数据结构定义已存在于根目录的`data_structures.py`文件中
   ...
   * **依赖模块导入**: 
     ...
     - **必须**区分模块的大小写，必须与**Upstream Dependency Implementations**中显示的模块名称完全一致，否则视为错误。如果**Upstream Dependency Implementations**中模块名为"Parser"，则必须使用`from Parser import *`，不能写成`from parser import *`。严禁被示例中出现的模块名误导。
   
   * **标准库导入**: 根据需要导入Python标准库（如typing, os, sys等）

4. **函数签名一致性**:
   * 调用依赖模块时，必须使用**Upstream Dependency Implementations**中显示的实际函数签名
